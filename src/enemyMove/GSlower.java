...
public class GSlower implements  IEnemyAI
{
    ...
    private boolean groundTileSenser(Enemy self, Tile[][] tiles)
    {
        ...
        else if(frontDownTile.getType() == Tile.TileType.CANHURT)
        {
            return false;
        }
        else
    ...
    public void senser(Tile[][] tiles, Enemy self, Actor actor) 
    {
        ...
        else if(groundDistanceSenser(self, actor, 150))
        {
    ...
    public void ahead(Enemy self) 
    {
        if(this.delay.isTrig())
        {
            ...
            if(self.dir() == Renderer.Dir.RIGHT && ( self.MoveSig() == Enemy.MoveSig.GO || self.MoveSig() == Enemy.MoveSig.ATTACK))
            {
                self.addVelX(SPEED);
            }
            else if(self.dir() == Renderer.Dir.LEFT && ( self.MoveSig() == Enemy.MoveSig.GO || self.MoveSig() == Enemy.MoveSig.ATTACK))
            {
                self.addVelX(-SPEED);
            }
